// Server
const URL = 'https://backend-coderhouse-challenge15-production.up.railway.app/';
...
window.addEventListener('load', async () => {
    const rawResponse = await fetch(`${URL}/users/me`);
...
formAuth.addEventListener('submit', async (event) => {
    ...
    if (event.submitter.id === 'btn-sign-in') {
        rawResponse = await fetch(`${URL}/auth/sign-in`, {
            headers: {
    ...
    // Sign-up
        ...
        rawResponse = await fetch(`${URL}/auth/sign-up`, {
            headers: {
                'Content-Type': 'application/json',
                'Content-Length': dataJSON.length
            },
...
btnSignOut.addEventListener('click', async () => {
    mainContainer.classList.add('d-none');
    const rawResponse = await fetch(`${URL}/auth/sign-out`, {
        headers: {
            'Content-Type': 'application/json'
        },
        method: 'POST',
...
socket.on('init-elements', async (data) => {
    ...
    await fetch(`${URL}/templates/card-images.hbs`)
...
btnServerInfo.addEventListener('click', async () => {
    if (!divServerInfo.classList.contains('d-none')) { divServerInfo.classList.add('d-none'); }
        ...
        const rawResponse = await fetch(`${URL}/info`);
        const response = await rawResponse.json();
...
btnGenerateNumbers.addEventListener('click', () => {
    ...
    window.location = `${URL}/api/randoms?qty=${qty}`;
});